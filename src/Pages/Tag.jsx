...
class Tag extends Component {
  ...
  fetchJobs() {
    ...
    if (localStorage.getItem("signed")) {
      ...
      fetch(`https://shielded-sands-39483.herokuapp.com/https://remotive.io/api/remote-jobs?tags=${params.tag}&limit=150`, {signal})
      .then(response => response.json())
      .then(jobs => { 
          this.setState({...this.state, jobs: jobs})
      })
  ...
  render() {
    ...
    return (
      <div className="App">
        <Helmet>
            <meta charSet="utf-8" />
            <title>Best {params.tag.replace(/[-]/g, " ")} remote Jobs - FromHome</title>
        </Helmet>
        <Header draw={draw} />
